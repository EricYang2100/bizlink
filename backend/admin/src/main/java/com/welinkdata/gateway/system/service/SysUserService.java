...
public class SysUserService {
    ...
    public Page selectUserList(SysUser user, PageRequest pageable) {
        ...
        if(StringUtils.isNotEmpty(user.getDeptId())){
            query.addCriteria(Criteria.where("deptAncestors").regex(".*"+user.getDeptId()+".*"));
        }
     ...
     * @return
     */
    public List<SysUser> selectUserAll(SysUser user){
        ExampleMatcher matcher = ExampleMatcher.matching()
                .withMatcher("deptAncestors", contains())
                .withMatcher("status",   exact())
                ;

        Example<SysUser> example = Example.of(user,matcher);

        List<SysUser> pc = userRepository.findAll(example);
    ...
    public SysUser insertUser(SysUser user){
        ...
        setDeptAncestors(user);
        return userRepository.insert(user);
    }

    /**
     * 冗余DeptAncestors=deptAncestor+","+deptId
     * @param user
     */
    private void setDeptAncestors(SysUser user) {
        if( StringUtils.isNotEmpty(user.getDeptId())){
